...
logit_mod_lasso <- logistic_reg(mixture=1) |>
  set_mode("classification") |>
  set_engine("glm")
```
...
```{r}
lg_rec <- recipe(TARGET ~ CONTRACT_TYPE_REVOLVING + EDUCATION_TYPE_HIGHER + FAMILY_STATUS_MARRIED + CNT_CHILDREN, data = train_random_fifty) |>
  step_mutate(TARGET = factor(TARGET)) |>
  step_dummy(all_nominal_predictors())
...
```{r}
# undersample
credit_sixty <- undersample_split(credit, 0.6)

# then, create sets
splits <- random_split(credit_sixty, train_prop = 0.7, val_prop = 0.1, seed = 123)
train_random_sixty <- splits$train
val_random_sixty <- splits$validation
test_random_sixty <- splits$test
```

```{r}
lg_rec <- recipe(TARGET ~ ., data = train_random_sixty) |>
  step_mutate(TARGET = factor(TARGET)) |>
  step_dummy(all_nominal_predictors())
...
cross_validation(data = train_random_sixty, model = "logistic", model_wkflow = logit_wkflow,
                 num_splits = 5, no_class = 0, bound = 0.5)
```
