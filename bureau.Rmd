...
```{r}
test_eval <- test_strat |>
  mutate(
    TARGET = factor(TARGET, levels = c(0, 1)),   # 1 = default (event)
    predictions = preds
  )

```

# Visualization Possibility 1: Confusion Matrix

```{r}
library(yardstick)
library(ggplot2)

cm <- conf_mat(test_eval, truth = TARGET, estimate = predictions)
...
# Plot Possibility 2: Bar Plot separated by Default & No Default for some classes
```{r}
library(ggplot2)
library(tidyr)
...
plot_df <- merged_ordered |>
  dplyr::select(TARGET, dplyr::all_of(selected_vars)) |>
  dplyr::mutate(
    TARGET = factor(TARGET, levels = c(0, 1),
                    labels = c("No Default", "Default"))
  ) |>
  tidyr::pivot_longer(
    cols = dplyr::all_of(selected_vars),
    names_to  = "predictor",
    values_to = "value"
  ) |>
...
ggplot(plot_df, aes(x = predictor, y = prop, fill = TARGET)) +
  ...
  labs(
    title = "Proportion with Feature = 1 by TARGET",
    x = "Predictor",
    y = "Proportion",
    fill = "TARGET"
  ) +
...
```{r}
ggplot(coef_df, aes(x = reorder(term, estimate), y = estimate, fill = estimate)) +
  geom_col() +
  coord_flip() +
  scale_fill_gradient2(low = "darkgreen", mid = "white", high = "red") +
  labs(
    title = "Log-Odds of Predictors",
    x = "Predictor",
    y = "Coefficient",
    fill = "Coefficient"
  ) +
