...
# then, create sets
splits <- random_split(df, train_prop = 0.6, val_prop = 0.2, seed = 123)
train_random_fifty <- undersample_split(splits$train)
...
train_strat_fifty <- undersample_split(strat_splits$train)
val_strat_fifty <- strat_splits$val
test_strat_fifty <- strat_splits$test
```

```{r}
lg_rec <- recipe(TARGET ~ ., data = train_strat_fifty) |>
  step_rm(SK_ID_CURR, INCOME_TO_CREDIT, INCOME_TO_REGION,
          CODE_GENDER, AGE_GROUPS, DAYS_EMPLOYED) |>
  step_mutate(TARGET = factor(TARGET)) |>
  step_dummy(all_nominal_predictors())
...
```{r}
lg_rec <- recipe(TARGET ~ INCOME_TO_CREDIT + INCOME_TO_REGION, 
